...
//这是用于堆式串的基本操作，使用时请手动改STRINGSIZE、STRINGADDON、STRINGEND定义，并按需修改TraverseString函数。
...
int ScanString(String *&s){
	//输入s的串内容，STRINGEND结束，成功返回0，失败返回1 
	char c;
	int flag,add;
	ClearString(s);
	flag=0;
...
void FilePrintString(String *&s,FILE *fp){
	//输出s的串内容至文件，STRINGEND结束 
	int flag;
	for(flag=0;flag<s->length;flag++){
		fprintf(fp,"%c",s->str[flag]);
	}
	fprintf(fp,"%c",STRINGEND);
	return;
}
