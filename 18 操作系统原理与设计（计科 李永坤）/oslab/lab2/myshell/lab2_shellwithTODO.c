...
int exec_builtin(int argc, char**argv, int *fd) {
    ...
    /* TODO: 添加和实现内置指令 */
    char *path;
    static char OLDPWD[MAX_BUF_SIZE] = {0};
    path = argv[1];
    if (strcmp(argv[0], "cd") == 0) {
        ...
        else if (argc == 2)
        {
            ...
            char oldpwd[MAX_BUF_SIZE] = {0};
            getcwd(oldpwd, MAX_BUF_SIZE - 1);
            if (chdir(path) == -1)
            {
                printf("cd: Can't visit path %s\n", path);
                return 0;
            }
...
int process_redirect(int argc, char** argv, int *fd) {
    ...
    while(i < argc) {
        ...
        if(strcmp(argv[i], ">") == 0) {
            //TODO: 打开输出文件从头写入
            tfd = open(argv[i+1], O_WRONLY | O_CREAT, 0777);
            if(tfd < 0) {
            ...
            //TODO: 打开输出文件追加写入
            tfd = open(argv[i+1], O_WRONLY | O_APPEND, 0777);
            if(tfd < 0) {
                printf("open '%s' error: %s\n", argv[i+1], strerror(errno));
            } else {
                //TODO:输出重定向
...
int execute(int argc, char** argv) {
    ...
    /* TODO:运行命令与结束 */
    execvp(argv[0], argv);
    printf("Can't execute file: %s\n", argv[0]);//execvp运行成功会直接结束进程，如果没有结束，说明没有运行成功
    close(fd[READ_END]);
    close(fd[WRITE_END]);
    exit(255);
}
...
int main() {
    ...
    while (1) {
        ...
        char *commands_with_pipe[128];
        /* TODO: 增加打印当前目录，格式类似"shell:/home/oslab ->"，你需要改下面的printf */
        char pwd[MAX_BUF_SIZE] = {0};
        getcwd(pwd, MAX_BUF_SIZE - 1);
        printf("shell: -> %s$ ", pwd);
        fflush(stdout);
        ...
        int cmd_count_with_pipe = split_string(cmdline, ";", commands_with_pipe);
        if(cmd_count_with_pipe == 0){
            continue;
        }
        else{
            for (size_t i = 0; i < cmd_count_with_pipe; i++)
            {
                char *commands[128];
                int cmd_count;
                /* 由管道操作符'|'分割的命令行各个部分，每个部分是一条命令 */
                /* 拆解命令行 */
                cmd_count = split_string(commands_with_pipe[i], "|", commands);
                if(cmd_count == 0) {
                    continue;
