...
static void *coalesce(void *bp)
{
    ...
    */
    ...
    {
        add_to_free_list(bp);   //直接加入空闲链表
    }
    else if (prev_alloc && !next_alloc) /*前块已分配，后块空闲*/
    {
        void *p_next_blk = NEXT_BLKP(bp);
        delete_from_free_list(p_next_blk);
        size_t new_size = GET_SIZE(HDRP(bp)) + GET_SIZE(HDRP(p_next_blk));
        PUT(HDRP(bp), PACK(new_size, GET_PREV_ALLOC(HDRP(bp)), 0));
        PUT(FTRP(p_next_blk), PACK(new_size, GET_PREV_ALLOC(HDRP(bp)), 0));
        add_to_free_list(bp);
    }
    else if (!prev_alloc && next_alloc) /*前块空闲，后块已分配*/
    {
        void *p_prev_blk = PREV_BLKP(bp);
        delete_from_free_list(p_prev_blk);
        size_t new_size = GET_SIZE(HDRP(bp)) + GET_SIZE(HDRP(p_prev_blk));
        PUT(HDRP(p_prev_blk), PACK(new_size, GET_PREV_ALLOC(HDRP(p_prev_blk)), 0));
        PUT(FTRP(bp), PACK(new_size, GET_PREV_ALLOC(HDRP(p_prev_blk)), 0));
        bp = p_prev_blk;
        add_to_free_list(bp);
    }
    else /*前后都是空闲块*/
    {
        ...
        delete_from_free_list(p_next_blk);
        delete_from_free_list(p_prev_blk);
        size_t new_size = GET_SIZE(HDRP(p_prev_blk)) + GET_SIZE(HDRP(bp)) + GET_SIZE(HDRP(p_next_blk));
        PUT(HDRP(p_prev_blk), PACK(new_size, GET_PREV_ALLOC(HDRP(p_prev_blk)), 0));   //设置新块的头
        PUT(FTRP(p_next_blk), PACK(new_size, GET_PREV_ALLOC(HDRP(p_prev_blk)), 0));   //设置新块的尾
        bp = p_prev_blk;
        add_to_free_list(bp);
    }
    return bp;
...
static void *find_fit_first(size_t asize)
{
    ...
    */
   char * search_free_p = free_listp;
   while (search_free_p != NULL)
   {
       if (GET_SIZE(HDRP(search_free_p)) >= asize)
       {
           return search_free_p;
       }
        search_free_p = GET_SUCC(search_free_p);
   }
...
static void* find_fit_best(size_t asize) {
    /* 
        ...
        HINT: asize 已经计算了块头部的大小
    */
    void *search_free_p = free_listp;
    void *best_fit_p = NULL;
    size_t best_fit_size = 2147483647;
    while (search_free_p != NULL)
    {
        if (GET_SIZE(HDRP(search_free_p)) >= asize && GET_SIZE(HDRP(search_free_p)) < best_fit_size )
       {
           best_fit_size = GET_SIZE(HDRP(search_free_p));
           best_fit_p = search_free_p;
       }
       search_free_p = GET_SUCC(search_free_p);
    }
    
    return best_fit_p; // 换成实际返回值
}
...
static void place(void *bp, size_t asize)
{
    ...
    */
    ...
    //不足空闲块的最小大小，全部分配
    if (free_size - asize < MIN_BLK_SIZE)
    {
        delete_from_free_list(bp);
        PUT(HDRP(bp), PACK(free_size, GET_PREV_ALLOC(HDRP(bp)), 1));
        void *head_next_bp = HDRP(NEXT_BLKP(bp));
        PUT(head_next_bp, PACK_PREV_ALLOC(GET(head_next_bp), 1));   //notify next block: I am alloced
        user_malloc_size += (free_size - WSIZE);
        return;
    }
    //否则，产生一个新的空闲块
    delete_from_free_list(bp);
    PUT(HDRP(bp), PACK(asize, GET_PREV_ALLOC(HDRP(bp)), 1));
    void* p_next_free = NEXT_BLKP(bp);
    PUT(HDRP(p_next_free), PACK(free_size - asize, 1, 0));
    PUT(FTRP(p_next_free), PACK(free_size - asize, 1, 0));
    void *head_next_bp = HDRP(NEXT_BLKP(p_next_free));
    PUT(head_next_bp, PACK_PREV_ALLOC(GET(head_next_bp), 0));   //notify next block: I am free
    coalesce(p_next_free);
    user_malloc_size += (asize - WSIZE); //已分配块的头部包括在asize里面，但不应该被算进user_malloc_size
    return;
}
