...
WORD alloc_clusters(FAT16 *fat16_ins, uint32_t n)
{
  ...
  /*** BEGIN ***/
  for (WORD Cluster = 0x0002; Cluster <= 0xFFEF; Cluster ++)
  {
    if (fat_entry_by_cluster(fat16_ins, Cluster) == 0x0000)
    {
  ...
  for (size_t i = 0; i < n; i++)
  {
    ...
    for (size_t j = FirstSectorofCluster; j < fat16_ins->Bpb.BPB_SecPerClus; j++)
    {
      memset(sector_buffer, 0x00, BYTES_PER_SECTOR * sizeof(BYTE));
      sector_write(fat16_ins->fd, FirstSectorofCluster + j, sector_buffer);
    }
    
    FILE *fd = fat16_ins->fd;
    WORD FATOffset = clusters[i] * 2;
    ...
    *((WORD *)&sector_buffer[FatEntOffset]) = clusters[i+1];
...
int fat16_mkdir(const char *path, mode_t mode) {
  ...
  if (findFlag == 1) {
    ...
    dir_entry_create(fat16_ins, sectorN, offset, paths[pathDepth - 1], 0x10, Cluster, 0);
    //找到新创建的目录的位置
    BYTE this_dir[11];
    BYTE last_dir[11];
    this_dir[0] = '.'; 
    ...
    dir_entry_create(fat16_ins, FirstSectorofCluster, 0, this_dir, 0x10, Cluster, 0);
    if (strcmp(prtPath, "/") == 0)
    {
      dir_entry_create(fat16_ins, FirstSectorofCluster, 32, last_dir, 0x10, 0xFFFF, 0);
    }
    else{
      DIR_ENTRY Dir;
      off_t offset_dir;
      find_root(fat16_ins, &Dir, prtPath, &offset_dir);
      dir_entry_create(fat16_ins, FirstSectorofCluster, 32, last_dir, 0x10, Dir.DIR_FstClusLO, 0);
    }
    /*** END ***/
  }
  return 0;
...
int fat16_rmdir(const char *path) {
  ...
  DIR_ENTRY curDir;
  off_t off;
  int res = find_root(fat16_ins, &Dir, path, &off);
  ...
  if (1)
  {
    ...
    if (findFlag == 1)
    {
      ...
      dir_entry_create(fat16_ins, sectorN, offset, paths[pathDepth - 1], 0x10, 0xffff, 0);
      return 0;
    }
  }
