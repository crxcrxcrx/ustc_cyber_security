                  ...
                  off_t offset, struct fuse_file_info *fi)
{
  ...
  if (strcmp(path, "/") == 0)
  {
    ...
    for (uint i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
    {
       ...
       **/
      /*** BEGIN ***/
      //计算目录项在sector_buffer中的偏移
      uint offset = (i - 1) * 32 - (RootDirCnt - 1) * 512;
      if (*(sector_buffer + offset) == 0x00)
      {
        break;
      }
      
      if (*(sector_buffer + offset + 0x0B) != 0x0F && *(sector_buffer + offset) != 0xE5) //跳过LFN和已删除
      {
        BYTE* filename;
        filename = path_decode(sector_buffer + offset);
  ...
  else  // 要查询的目录不是根目录
  {
    ...
    for (uint i = 1; Dir.DIR_Name[0] != 0x00; i++)
    {
      ...
      /*** BEGIN ***/
      //计算目录项在sector_buffer中的偏移
      uint offset = (i - 1) * 32 - (DirSecCnt - 1) * 512;
      if (*(sector_buffer + offset) == 0x00)
      {
        break;
      }
      if (*(sector_buffer + offset + 0x0B) != 0x0F && *(sector_buffer + offset) != 0xE5)
      {
        BYTE* filename;
        filename = path_decode(sector_buffer + offset);
               ...
               struct fuse_file_info *fi)
{
  ...
  /*** BEGIN ***/
  size_t sec_per_clus = fat16_ins->Bpb.BPB_SecPerClus;

  DIR_ENTRY Dir;
  off_t offset_dir;
  find_root(fat16_ins, &Dir, path, &offset_dir);
  size_t beginBytes =  offset;
  size_t endBytes = offset + size - 1;
  
  WORD BeginClusterN;
  BeginClusterN = Dir.DIR_FstClusLO; //目录项中存储了我们要读取的第一个簇的簇号

  WORD EndClusterN;
  EndClusterN = Dir.DIR_FstClusLO;
  for (size_t i = 0; i < endBytes / (sec_per_clus * BYTES_PER_SECTOR); i++)
  {
    EndClusterN = fat_entry_by_cluster(fat16_ins, EndClusterN);
    if (EndClusterN >= 0xFFF8)
    {
      break;
    }
  }
  BYTE* file_buf = malloc((1 << 25) * sizeof(BYTE));
  size_t file_buf_offset = 0;
  for (WORD Cluster = BeginClusterN; Cluster != fat_entry_by_cluster(fat16_ins, EndClusterN); Cluster = fat_entry_by_cluster(fat16_ins, Cluster))
  {
    WORD FirstSectorofCluster;  // 该簇的第一个扇区号
    FirstSectorofCluster = ((Cluster - 2) * fat16_ins->Bpb.BPB_SecPerClus) + fat16_ins->FirstDataSector;
    for (size_t i = 0; i < sec_per_clus; i++)
    {
      WORD SectorNum = FirstSectorofCluster + i;
      sector_read(fat16_ins->fd, SectorNum, file_buf + file_buf_offset);
      file_buf_offset += BYTES_PER_SECTOR;
    }
  }
  memcpy(buffer, file_buf + beginBytes, size);
  free(file_buf);
  /*** END ***/
  return size;
}
...
int fat16_mknod(const char *path, mode_t mode, dev_t devNum)
{
  ...
  BYTE sector_buffer[BYTES_PER_SECTOR];
  DWORD sectorNum;
  int offset, findFlag = 0, RootDirCnt = 1, DirSecCnt = 1, sectorN;
  WORD ClusterN, FatClusEntryVal, FirstSectorofCluster;
  ...
  if (strcmp(prtPath, "/") == 0)
  {
    ...
    {
      //计算目录项在sector_buffer中的偏移
      uint offset_in_sector = (i - 1) * 32 - (RootDirCnt - 1) * 512;
      if ((*(sector_buffer + offset_in_sector) == 0xE5 || *(sector_buffer + offset_in_sector) == 0x00) && findFlag == 0)
      {
        findFlag = 1;
        offset = offset_in_sector;
        sectorN = fat16_ins->FirstRootDirSecNum + RootDirCnt - 1;
        if (*(sector_buffer + offset_in_sector) == 0x00)
        {
          break;
        }
      }
      if (*(sector_buffer + offset_in_sector + 0x0B) != 0x0F && *(sector_buffer + offset_in_sector) != 0xE5) //跳过LFN和已删除
      {
        BYTE* filename;
        filename = path_decode(sector_buffer + offset_in_sector);
        if (strcmp(filename, orgPaths[pathDepth - 1]) == 0)
        {
          findFlag = 0;
          return -EEXIST;
        }
  ...
  else
  {
    ...
    for (uint i = 1; Dir.DIR_Name[0] != 0x00; i++)
    {
      uint offset_in_sector = (i - 1) * 32 - (DirSecCnt - 1) * 512;
      if ((*(sector_buffer + offset_in_sector) == 0xE5 || *(sector_buffer + offset_in_sector) == 0x00) && findFlag == 0)
      {
        findFlag = 1;
        offset = offset_in_sector;
        sectorN = FirstSectorofCluster + DirSecCnt - 1;
        if (*(sector_buffer + offset_in_sector) == 0x00)
        {
          break;
        }
      }
      if (*(sector_buffer + offset_in_sector + 0x0B) != 0x0F && *(sector_buffer + offset_in_sector) != 0xE5)
      {
        BYTE* filename;
        filename = path_decode(sector_buffer + offset_in_sector);
        if (strcmp(filename, orgPaths[pathDepth - 1]) == 0)
  ...
  if (findFlag == 1) {
    ...
    dir_entry_create(fat16_ins, sectorN, offset, paths[pathDepth - 1], 0x20, 0xffff, 0);
    /*** END ***/
  }
  return 0;
}
...
int dir_entry_create(FAT16 *fat16_ins, int sectorNum, int offset, char *Name, BYTE attr, WORD firstClusterNum, DWORD fileSize)
{
  ...
  memset(entry_info + 0x0B, attr, sizeof(BYTE));
  ...
  /*** BEGIN ***/
  memcpy(entry_info + 0x1A, &firstClusterNum, 2 * sizeof(BYTE));
  memcpy(entry_info + 0x1C, &fileSize, 4 * sizeof(BYTE));
  /*** END ***/
  ...
  memcpy(sector_buffer + offset, entry_info, 32 * sizeof(BYTE));
...
int free_cluster(FAT16 *fat16_ins, int ClusterNum)
{
  ...
  WORD FatEntOffset = FATOffset % fat16_ins->Bpb.BPB_BytsPerSec;
  sector_read(fat16_ins->fd, FatSecNum, sector_buffer);
  sector_buffer[FatEntOffset] = 0x0000;
...
int fat16_unlink(const char *path)
{
  ...
  /*** BEGIN ***/
  for (WORD ClusterN = Dir.DIR_FstClusLO; ClusterN != 0xFFFF; ClusterN = free_cluster(fat16_ins, ClusterN));
