...
// 定义要输出的文件
#define OUTPUT_FILE "/home/pocket/oslab/lab3/lab3-part2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffff81cb0780;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffff81c94160;
...
static void scan_vma(void)
{
    ...
    if (mm)
    {
        ...
        struct vm_area_struct* mmap_iter = mm->mmap;
        while (mmap_iter != NULL)
        {
            my_task_info.vma_cnt ++;
            mmap_iter = mmap_iter->vm_next;
        }
        mmput(mm);
    }
}
...
static void print_mm_active_info(void)
{
    ...
    if (mm)
    {
        ...
        while (vma != NULL)
        {
            size_t vm_length = vma->vm_end - vma->vm_start;
            size_t vm_page_num = vm_length / PAGE_SIZE;
            size_t offset = 0;
            while (offset < vm_page_num)
            {
                struct page *page = mfollow_page(vma, vma->vm_start + offset * PAGE_SIZE, FOLL_GET);
                unsigned long vm_flags;
                if (!IS_ERR_OR_NULL(page))
                {
                    if (mpage_referenced(page, 0, (struct mem_cgroup *)(page->memcg_data), &vm_flags))
                    {
...
static unsigned long virt2phys_withVMA(struct vm_area_struct *vma, unsigned long virt)
{
    struct page *page = NULL;
    // TODO: 多级页表遍历：pgd->pud->pmd->pte，然后从pte到page，最后得到pfn
    page = mfollow_page(vma, virt, FOLL_GET);
    if (page)
    {
...
static unsigned long virt2phys(struct mm_struct *mm, unsigned long virt)
{
    struct page *page = NULL;
    // TODO: 多级页表遍历：pgd->pud->pmd->pte，然后从pte到page，最后得到pfn
    pgd_t* pgd_pointer = pgd_offset(mm, virt);
    pud_t* pud_pointer = (pgd_none(*pgd_pointer) || pgd_bad(*pgd_pointer)) ? NULL : pud_offset((p4d_t*)pgd_pointer, virt);
    pmd_t* pmd_pointer = (pud_none(*pud_pointer) || pud_bad(*pud_pointer)) ? NULL : pmd_offset(pud_pointer, virt);
    pte_t* pte_pointer = (pmd_none(*pmd_pointer) || pmd_bad(*pmd_pointer)) ? NULL : pte_offset_kernel(pmd_pointer, virt);
    page = pte_present(*pte_pointer) ? pte_page(*pte_pointer) : NULL;
    if (page)
    {
        return page_to_pfn(page);
    }
...
static void traverse_page_table(struct task_struct *task)
{
    ...
    if (mm)
    {
        ...
        while (vma != NULL)
        {
            ...
            while (offset < vm_page_num)
            {
                size_t virt_addr = vma->vm_start + offset * PAGE_SIZE;
                if (virt2phys(mm, virt_addr))
                {
...
static void print_seg_info(void)
{
    ...
    if (ktest_func == 4)
    {
        ...
        struct vm_area_struct *start_vma = find_vma(mm, mm->start_data);
        struct vm_area_struct *end_vma = find_vma(mm, mm->end_data);
        struct vm_area_struct *vma = start_vma;
        struct vm_area_struct *end_vma_next;
        if (end_vma != NULL)
        {
            end_vma_next = end_vma->vm_next;
        }
        else{
            end_vma_next = end_vma;
        }
        
        while (vma != end_vma_next)
        {
            size_t vm_length = vma->vm_end - vma->vm_start;
            size_t vm_page_num = vm_length / PAGE_SIZE;
            size_t offset = 0;
        ...
        /* Look up the first VMA which satisfies  addr < vm_end,  NULL if none. */
        // extern struct vm_area_struct * find_vma(struct mm_struct * mm, unsigned long addr);
        struct vm_area_struct *start_vma = find_vma(mm, mm->start_code);
        struct vm_area_struct *end_vma = find_vma(mm, mm->end_code);
        struct vm_area_struct *vma = start_vma;
        struct vm_area_struct *end_vma_next;
        if (end_vma != NULL)
        {
