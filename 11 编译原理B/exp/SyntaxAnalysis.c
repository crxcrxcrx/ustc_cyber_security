...
static int Prod_S()
{
...
#endif
		//这里我和室友一起讨论得出的代码, 我们这块的代码可能重复了, 希望助教不要误会 
		match(SYN_IF);
		match(SYN_PAREN_L);
		bval = Prod_B();
		match(SYN_PAREN_R);
	...
	else if (lookahead.token == SYN_WHILE)
	{
		...
		Prod_S();
		match(SYN_BRACE_R);
		if (run_status == 1 || (run_status == 2 && continue_status == 1))
		{
			continue_status = 0;
			run_status = 1;
			//这里相当于continue的操作 
...
static int Prod_TB1(int bval1)
{
	...
	if (lookahead.token == SYN_AND)
	{
		...
		printf("SYN: TB1-->&& TB1\n");
#endif
		//printf("&&传来的参数bval1是%d\n",bval1);
		match(SYN_AND);
		bval2 = Prod_FB();//修改bval1为bval2
		bval1 = (run_status == 1 && (bval1 == 1 && bval2 == 1)) ? 1 : 0;
		bval2 = Prod_TB1(bval1);
		return(bval2);
	}
