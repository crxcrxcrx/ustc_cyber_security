...
- 计科开课
- 三个班：郑启龙、李诚、张昱。默认置课应该是张昱班。作者本人换到了李诚班。李诚班和张昱班实验和考试要求都类似，郑启龙班未知。
- 教学内容
  - 词法分析
  - 语法分析
  - 中间代码生成
  ...
  - 代码生成
  - 一些堆栈管理的相关知识
- 实验内容：实验特别难（完成一个cminus-f语言的编译器），请大家做好心理准备！！
  - 词法分析和语法分析：使用Flex和Bison完成词法分析和语法分析，将cminus-f（简化版的c语言）代码转换成语法分析树
  - llvm初体验：了解llvm中间代码
  - 中间代码生成：将语法分析树转换成抽象语法树（助教已经实现），将抽象语法树转换成llvm中间代码（自己实现）
  - 代码优化：优化llvm中间代码（在llvm IR的层面上），需要实现：提取循环不变量、常量传播、死代码删除，活跃变量分析。
  - （选做）代码生成
- 实验占比很高（60%），平时成绩（主要是作业）10%，期中期末各15%
- 考试可能会比较难，以下为考试重点：
  - 词法正则表达式和有限自动机之间的转化
  - 有限自动机的优化（NFA和DFA之间的转化，DFA的简化）
  - 每一种语法规则的概念和语法分析方法，画语法树
  - 中间代码（抽象语法树，有向无环图，三地址代码）的生成方法和转化方法
  - 代码优化的算法（常量传播，死代码删除，公共子表达式提取，到达定值分析、活跃变量分析等）
  - 堆栈管理（函数在栈空间中的结构）
  - 结构体的内存占用方式
- 考试形式基本上都是大题
...
- 计科开课
- 三个班：顾乃杰、李向阳、徐云。默认置课应该是徐云，作者本人换到了顾乃杰。
- 教学内容
  - 算法分析，复杂度理论
  - 排序算法：基于比较的排序，基于数值的排序
  - 顺序统计量
  - 高级数据结构
    - 二分检索树
    - 红黑树
...
- 计科开课
- 两个班：李永坤、陈香兰。默认置课应该是陈香兰，作者本人换到李永坤班。两个班实验内容差距较大。
- 课程内容
  - 进程管理
    - 进程概念
    - 进程间通信
...
-   教学内容: 计算机安全基础理论, 操作系统安全, 应用安全, 云计算安全, 网络安全
    -   内容繁杂, 老师考前会讲什么考什么不考
-   有点名, 但来的人还是很少
-   作业考试比较靠近, 可以参考
